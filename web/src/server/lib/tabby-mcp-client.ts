...
class TabbyMcpClient {
	...
	async callTool<T = unknown>(
		...
		while (true) {
			try {
				...
				attempt++;
				if (attempt >= 2) {
					throw new TabbyMcpError(
						`MCP call ${name} failed: ${(err as Error).message}`,
						err,
					);
				}
			}
	...
	// ---- Tabs -------------------------------------------------------------
	listTabs = () =>
		this.callTool<{ tabs: TabbyListTabsEntry[]; count: number }>("list_tabs");
	selectTab = (args: { tabId?: string; tabIndex?: number }) =>
		this.callTool("select_tab", args);
	closeTab = (args: { tabId?: string; tabIndex?: number }) =>
		this.callTool("close_tab", args);
	...
	quickConnect = (args: { target: string }) =>
		this.callTool("quick_connect", args);
	...
	sftpDelete = (args: { sessionId?: string; path: string }) =>
		this.callTool("sftp_delete", args);
	sftpMkdir = (args: { sessionId?: string; path: string }) =>
		this.callTool("sftp_mkdir", args);
	sftpRename = (args: { sessionId?: string; from: string; to: string }) =>
		this.callTool("sftp_rename", args);
	sftpStat = (args: { sessionId?: string; path: string }) =>
		this.callTool("sftp_stat", args);
...
export const tabbyMcp = new TabbyMcpClient(
	process.env.TABBY_MCP_URL ?? DEFAULT_ENDPOINT,
);
